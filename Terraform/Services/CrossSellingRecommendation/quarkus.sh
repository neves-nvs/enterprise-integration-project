...
sudo docker pull "$DOCKERHUB_USER"/crosssellingrecommendation:1.0.0-SNAPSHOT

sudo docker run -d --name crosssellingrecommendation \
    -p 8080:8080 \
    -e MYSQL_DATABASE_URL="$MYSQL_DATABASE_URL" \
    -e KAFKA_BROKER_URL="$KAFKA_BROKER_URL" \
    "$DOCKERHUB_USER"/crosssellingrecommendation:1.0.0-SNAPSHOT
